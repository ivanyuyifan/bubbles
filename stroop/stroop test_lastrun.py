...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/fafaya/Desktop/stroop/stroop test_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('/Users/fafaya/Desktop/stroop/stroop.xlsx'), 
        seed=None, 
    )
    ...
    for thisTrial in trials:
        ...
        # create an object to store info about Routine experiment_trial
        experiment_trial = data.Routine(
            name='experiment_trial',
            components=[experimenttext, key_resp_2],
        )
        experiment_trial.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        experimenttext.setColor(colour, colorSpace='rgb')
        experimenttext.setText(word)
        # create starting attributes for key_resp_2
        key_resp_2.keys = []
        key_resp_2.rt = []
        _key_resp_2_allKeys = []
        # store start times for experiment_trial
        experiment_trial.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        experiment_trial.tStart = globalClock.getTime(format='float')
        experiment_trial.status = STARTED
        thisExp.addData('experiment_trial.started', experiment_trial.tStart)
        experiment_trial.maxDuration = None
        # keep track of which components have finished
        experiment_trialComponents = experiment_trial.components
        for thisComponent in experiment_trial.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        if isinstance(trials, data.TrialHandler2) and thisTrial.thisN != trials.thisTrial.thisN:
            continueRoutine = False
        experiment_trial.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine and routineTimer.getTime() < 3.5:
            ...
            # if experimenttext is starting this frame...
            if experimenttext.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                # keep track of start time/frame for later
                experimenttext.frameNStart = frameN  # exact frame index
                experimenttext.tStart = t  # local t and not account for scr refresh
                experimenttext.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(experimenttext, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'experimenttext.started')
                # update status
                experimenttext.status = STARTED
                experimenttext.setAutoDraw(True)
            
            # if experimenttext is active this frame...
            if experimenttext.status == STARTED:
                # update params
                pass
            
            # if experimenttext is stopping this frame...
            if experimenttext.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > experimenttext.tStartRefresh + 3-frameTolerance:
                    # keep track of stop time/frame for later
                    experimenttext.tStop = t  # not accounting for scr refresh
                    experimenttext.tStopRefresh = tThisFlipGlobal  # on global time
                    experimenttext.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'experimenttext.stopped')
                    # update status
                    experimenttext.status = FINISHED
                    experimenttext.setAutoDraw(False)
            ...
            if key_resp_2.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_2.getKeys(keyList=['left','right','down'], ignoreKeys=["escape"], waitRelease=False)
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                experiment_trial.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in experiment_trial.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        for thisComponent in experiment_trial.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for experiment_trial
        experiment_trial.tStop = globalClock.getTime(format='float')
        experiment_trial.tStopRefresh = tThisFlipGlobal
        thisExp.addData('experiment_trial.stopped', experiment_trial.tStop)
        # check responses
        ...
        if experiment_trial.maxDurationReached:
            routineTimer.addTime(-experiment_trial.maxDuration)
        elif experiment_trial.forceEnded:
            routineTimer.reset()
        else:
